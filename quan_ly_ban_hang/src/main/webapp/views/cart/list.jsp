...
<div class="container mt-4">
  ...
  <table class="table table-striped table-bordered">
    ...
    <tr>
      <th>Stt</th>
      <th>Tên sản phẩm</th>
      <th>Ảnh</th> <!-- Cột mới cho ảnh sản phẩm -->
      <th>Số lượng</th>
      <th>Giá</th>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${cart.getProductCarts()}" var="ca" varStatus="status">
      <tr>
        <td>${status.index + 1}</td> <!-- Đánh số thứ tự bắt đầu từ 1 -->
        <td>${ca.getKey().getName()}</td>
        <td>
          <img src="${ca.getKey().getImageUrl()}" alt="${ca.getKey().getName()}" class="img-thumbnail" style="width: 100px; height: auto;">
        </td>
        <td>${ca.getValue()}</td>
        <td>${ca.getKey().getPrice()} VNĐ</td>
      </tr>
    </c:forEach>
    </tbody>
  </table>
